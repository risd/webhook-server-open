...
function configure (config) {
  ...
  function runBuildEmitter ({ builtFolder, bucketSpec }) {
    const buildEvent = 'build:document-written:./.build/'
    return miss.through.obj(function (cmd, enc, next) {
      ...
      p.on('message', (msg) => {
        console.log(msg)
        if (!msg.startsWith(buildEvent)) return
        let builtFile = msg.trim().slice(buildEvent.length)
          ...
          function buildUploadSite ( options ) {
            ...
            return miss.through.obj( function ( args, enc, next ) {
              ...
              miss.pipe(
                ...
                runBuildEmitter( buildEmitterOptions ),  // pushes { builtFile, builtFilePath, bucket }
                buildSitemap( siteMapOptions ),          // through stream, writes { builtFile, builtFilePath, bucket } on end
                buildRobotsTxt( robotsTxtOptions ),      // through stream, writes { builtFile, builtFilePath, bucket } on end
